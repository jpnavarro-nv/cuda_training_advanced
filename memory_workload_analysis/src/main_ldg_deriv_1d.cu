...
template <int radius>
__global__ void ldg_deriv_copy(float *__restrict__ input, float *__restrict__ output, int n0, int n1, int n0_pad)
{
...
{
    // Implement a first-order derivative on the input array along the slow dimension
    // Watch out for the indices (we did not pad the input array in the slow direction)
}
